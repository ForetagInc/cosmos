...
const nativeSelectVariants = tv({
	base: 'peer w-full appearance-none bg-none rounded-md border-0 bg-[var(--select-bg)] text-[var(--select-fg)] shadow-[var(--select-shadow)] transition-[color,background-color,border-color,box-shadow] hover:bg-[var(--select-bg-hover)] focus:outline-none focus-visible:shadow-[var(--select-shadow-focus)] focus-visible:ring-0 disabled:cursor-not-allowed disabled:bg-[var(--select-disabled-bg)] disabled:text-[var(--select-disabled-fg)] invalid:shadow-[var(--input-shadow-error)] aria-[invalid=true]:shadow-[var(--input-shadow-error)] [&::-ms-expand]:hidden',
	variants: {
		size: {
