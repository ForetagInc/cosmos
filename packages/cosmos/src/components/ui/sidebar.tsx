		...
		return (
			...
			>
				...
				>
					<div
						data-sidebar="sidebar"
						className="flex h-full w-full flex-col bg-sidebar group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-[var(--sidebar-stroke)] group-data-[variant=floating]:shadow-[var(--sidebar-panel-shadow)]"
					>
						{children}
					</div>
				</div>
...
>(({ className, onClick, ...props }, ref) => {
	...
	return (
		<Button
			...
			{...props}
		>
			<i className="ti ti-layout-sidebar-left-expand text-lg" />
...
>(({ className, render, ...props }, ref) =>
	useRender({
		...
		props: {
			...
			className: cn(
				'flex h-7 shrink-0 items-center rounded-md px-2.5 font-medium text-[11px] text-sidebar-foreground/85 uppercase tracking-[0.03em] outline-none ring-sidebar-ring transition-[margin,opacity] duration-200 ease-linear focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0',
...
>(({ className, ...props }, ref) => (
	<ul
		...
		className={cn(
			'mx-2 flex min-w-0 translate-x-px flex-col gap-0.5 px-3 py-0.5',
			'group-data-[collapsible=icon]:hidden',
			className,
		)}
		{...props}
